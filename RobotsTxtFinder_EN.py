...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Check if robot.txt is accessible for a given set of URLs')
    parser.add_argument('urls_file', help='path to file containing URLs to check (e.g. /path/to/urls.txt)')
    parser.add_argument('--user-agent', '-ua', default='Python requests', help='user agent to use (default: Python requests)')
    parser.add_argument('--concurrency', '-c', type=int, default=10, help='maximum number of concurrent requests (default: 10)')
    parser.add_argument('--timeout', '-t', type=int, default=5, help='maximum timeout for each request in seconds (default: 5)')
    parser.add_argument('--sync', action='store_true', help='use synchronous requests instead of async (default: False)')
